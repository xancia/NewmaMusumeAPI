...
ORDER BY `srm`.`chara_id`, `sr`.`relation_type`;
...
AS
SELECT
  `race_instance`.`id` AS `race_instance_id`,
...
AS
SELECT
  `tssb`.`id` AS `team_stadium_score_bonus_id`,
...
AS
SELECT
  `scue`.`id` AS `support_card_unique_effect_id`,
  `scue`.`lv` AS `lv`,
...
AS
SELECT
  `sesd`.`id` AS `story_event_story_id`,
...
AS
SELECT DISTINCT
  `sem`.`id` AS `story_event_mission_id`,
...
AS
SELECT
  `sed`.`story_event_id` AS `story_event_id`,
...
AS
SELECT
  `smcg`.`id` AS `single_mode_chara_grade_id`,
...
AS
SELECT
  `iet`.`id` AS `item_exchange_top_id`,
