...
namespace UmaMusumeAPI.Controllers.StoredProcedures
{
    ...
    public class SpSuccessionGrandparentRecommendationController : ControllerBase
    {
        ...
        public async Task<
            ActionResult<IEnumerable<SpSuccessionGrandparentRecommendation>>
        > GetSpSuccessionGrandparentRecommendation(int child, int parent)
        {
            if (child == 0 || parent == 0)
            {
                return NotFound();
            ...
            )
            {
                ...
                using (MySqlCommand command = conn.CreateCommand())
                {
                    ...
                    {
                        List<SpSuccessionGrandparentRecommendation> grandparentRecommendations =
                            new List<SpSuccessionGrandparentRecommendation>();

                        while (await dr.ReadAsync())
                        {
                            grandparentRecommendations.Add(
                                new SpSuccessionGrandparentRecommendation
                                {
                                    GrandparentRecommendationId = int.Parse(
                                        dr["grandparent_recommendation_id"].ToString()
                                    ),
                                    GrandparentCompatibility = int.Parse(
                                        dr["grandparent_compatibility"].ToString()
                                    ),
                                }
                            );
                        }
